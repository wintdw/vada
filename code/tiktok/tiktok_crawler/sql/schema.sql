...
CREATE TABLE `CrawlHistory` (
  ...
  `crawl_status` enum('pending','in_progress','success','failed','skipped') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'pending',
  `crawl_error` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
  `crawl_duration` int DEFAULT NULL,
  `crawl_data_number` int DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `crawl_id` (`crawl_id`)
