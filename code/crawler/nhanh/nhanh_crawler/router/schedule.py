...
@router.post(
    "/v1/schedule/{index_name}/crawl",
    response_model=NhanhCrawlInfoResponse,
    tags=["Schedule"],
)
    ...
    try:
        ...
        for item in crawl_info:
            crawl_history = await insert_crawl_history(
                NhanhCrawlHistory(business_id=item.business_id)
            )
            logger.info(crawl_history)

            history_id = crawl_history.history_id

            nhanh_platform_crawl.labels(
                index_name=item.index_name, business_id=item.business_id
            ).inc()
            ...
            if not item.last_crawl_time:
                ...
                for start_offset, end_offset in date_ranges:
                    crawl_response = await crawl_nhanh_data(
                        ...
                        item.business_id,
                        item.access_token,
                        (datetime.now() - timedelta(days=end_offset)).strftime(
                            "%Y-%m-%d"
                        ),
                        (datetime.now() - timedelta(days=start_offset)).strftime(
                            "%Y-%m-%d"
                        ),
                    )
            ...
            nhanh_platform_crawl_success.labels(
                index_name=item.index_name, business_id=item.business_id
            ).inc()
        ...
        crawl_history = await update_crawl_history(
            history_id,
            NhanhCrawlHistory(
                business_id=item.business_id, crawl_status="failed", crawl_error=str(e)
            ),
        )
        logger.error(crawl_history)

    finally:
        return NhanhCrawlInfoResponse(status=200, message="Success")
