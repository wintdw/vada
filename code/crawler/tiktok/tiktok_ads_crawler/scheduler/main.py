...
async def schedule_tiktokad_crawl_job(
    crawl_id: str,
    first_crawl: bool = False,  # Whether to start the crawl immediately
):
    tasks = []
    if first_crawl:
        tasks.append(asyncio.create_task(crawl_first_tiktokad(crawl_id=crawl_id)))

    tasks.append(asyncio.create_task(crawl_daily_tiktokad_scheduler(crawl_id=crawl_id)))
    ...
    async def update_jobs():
        ...
        for info in crawl_infos:
            ...
            task = asyncio.create_task(
                schedule_tiktokad_crawl_job(crawl_id=crawl_id, first_crawl=first_crawl)
            )
