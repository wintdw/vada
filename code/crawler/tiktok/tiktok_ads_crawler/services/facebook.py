...
def handle_failure(response: FacebookResponse):
    """Handle failure callback for Facebook API batch requests."""
    exception = response.error()
    logger.error(f"Error: {exception}")


def handle_success(response: FacebookResponse):
...
async def fetch_ads_for_accounts(ad_accounts: list[AdAccount]):
    ...
    """
    #ads_api = FacebookAdsApi.get_default_api()
