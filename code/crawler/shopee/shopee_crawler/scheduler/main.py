...
async def add_shopee_refresh_job(
    ...
    try:
        ...
        scheduler.add_job(
            ...
            id=job_id,
            name=f"Refresh Shopee Token for Shop: {account_name} ({account_id}) for Vada UID: {vada_uid}",
            replace_existing=True,
            misfire_grace_time=30,
            max_instances=1,
        )
        logging.info(
            f"[Scheduler] Added Shopee Token refresh job for Shop: {account_name} ({account_id}) for Vada UID: {vada_uid}"
        )
    except Exception as e:
        logging.error(
            f"[Scheduler] Error adding Shopee Token refresh job for Shop: {account_name} ({account_id}) for Vada UID: {vada_uid}: {str(e)}",
            exc_info=True,
        )


async def add_shopee_shop_crawl_job(
    scheduler: AsyncIOScheduler,
    access_token: str,
    index_name: str,
    job_id: str,
    ...
    try:
        ...
        scheduler.add_job(
            ...
            id=job_id,
            name=f"Fetch Shopee Order for Shop: {account_name}, Index: {index_name} every {crawl_interval} minutes",
            replace_existing=True,
            misfire_grace_time=30,
            max_instances=1,
        )
        ...
        logging.info(
            f"[Scheduler] Added Shopee Order job for Shop: {account_name}, Index: {index_name} every {crawl_interval} minutes"
        )
    except Exception as e:
        logging.error(
            f"[Scheduler] Error adding Shopee Order job for Shop: {account_name}, Index: {index_name}: {str(e)}",
            exc_info=True,
        )
...
async def init_scheduler():
    ...
    async def update_jobs():
        # Fetch crawl info specific to Shopee Shop
        crawl_info = await get_crawl_info("shopee_shop")
        ...
        for info in crawl_info:
            ...
            if refresh_job_id not in current_jobs:
                await add_shopee_shop_crawl_job(
                    scheduler=scheduler,
                    access_token=access_token,
                    index_name=index_name,
                    job_id=crawl_job_id,
        ...
        for left_job_id in current_jobs:
            ...
            if left_job_id != "update_jobs":
                ...
                logging.info(
                    f"[Scheduler] Removed Shopee Order job with ID: {left_job_id} as it is no longer valid"
                )
    ...
    scheduler.add_job(
        ...
        id="update_jobs",
        name="Update Shopee Order jobs every 1 minute",
